...
# Daily installs by platform and source
install_platform = events_df[events_df['event_name'] == 'install'] \
    .groupby([events_df['event_timestamp'].dt.date, 'platform', 'source']) \
    .size() \
    .reset_index(name='installs')
install_platform['date'] = pd.to_datetime(install_platform['event_timestamp'])

# Pivot for easier plotting
pivot_ios = install_platform[install_platform['platform'] == 'ios'].pivot(index='date', columns='source', values='installs').fillna(0)
pivot_android = install_platform[install_platform['platform'] == 'android'].pivot(index='date', columns='source', values='installs').fillna(0)

# Plot iOS organic vs paid
plt.figure(figsize=(14,4))
plt.plot(pivot_ios.index, pivot_ios['Organic'], label='iOS Organic', color='green')
plt.plot(pivot_ios.index, pivot_ios['Google Brand Search'] + pivot_ios['Meta'] + pivot_ios["TikTok"] + pivot_ios["Google App Campaigns"] + pivot_ios["Referral"], label='iOS Paid (sum)', color='red')
plt.legend()
plt.title('iOS Installs: Organic vs Paid (last‑click)')
plt.show()
...
# Ensure daily is regularly spaced and fill missing with 0
daily_ts = daily.set_index('date').asfreq('D', fill_value=0)

# Cross-correlation: organic_installs vs total_paid_spend
ccf_vals = ccf(daily_ts['organic_installs'], daily_ts['total_paid_spend'], adjusted=False)
lags = np.arange(len(ccf_vals))

plt.figure(figsize=(10,4))
plt.stem(lags[:21], ccf_vals[:21])
plt.xlabel('Lag (days)')
plt.ylabel('Cross-correlation')
plt.title('Cross-correlation: Organic Installs vs. Paid Spend')
plt.show()


# COMMAND ----------

# Extract campaign type from campaign_name (e.g., contains 'Brand', 'Awareness', 'Retargeting')
spend_df['campaign_type'] = spend_df['campaign_name'].str.extract('(Brand|Awareness|Retargeting|Performance)')

# Daily spend by campaign type
type_spend = spend_df.groupby(['date', 'campaign_type'])['spend'].sum().reset_index()
pivot_type = type_spend.pivot(index='date', columns='campaign_type', values='spend').fillna(0)

# Plot alongside organic installs
fig, ax1 = plt.subplots(figsize=(14,6))
ax1.plot(pivot_type.index, pivot_type['Brand'], label='Brand Spend', color='orange')
ax1.plot(pivot_type.index, pivot_type['Awareness'], label='Awareness Spend', color='purple')
ax1.set_ylabel('Spend ($)')
ax1.legend(loc='upper left')

ax2 = ax1.twinx()
ax2.plot(daily_ts.index, daily_ts['organic_installs'], color='green', label='Organic Installs')
ax2.set_ylabel('Organic Installs')
ax2.legend(loc='upper right')

plt.title('Brand/Awareness Spend vs Organic Installs')
plt.show()

# COMMAND ----------

# MAGIC %md
# MAGIC ## Organic Traffic – Relationship Between Paid Spend and Organic Installs
...
for lag in range(1, 8):
    daily[f'paid_spend_lag{lag}'] = daily['total_paid_spend'].shift(lag)

# Drop NaN rows from shifting
model_data = daily.dropna().copy()

# Define X (with a constant) and y
X = model_data[[f'paid_spend_lag{lag}' for lag in range(1,8)]]
X = sm.add_constant(X)
y = model_data['organic_installs']
...
# Daily organic installs by platform
organic_ios = (events_df[(events_df['event_name'] == 'install') & 
                         (events_df['source'] == 'Organic') & 
                         (events_df['platform'] == 'ios')]
               .groupby(events_df['event_timestamp'].dt.date)
               .size()
               .reset_index(name='organic_ios'))
organic_ios['date'] = pd.to_datetime(organic_ios['event_timestamp'])

organic_android = (events_df[(events_df['event_name'] == 'install') & 
                             (events_df['source'] == 'Organic') & 
                             (events_df['platform'] == 'android')]
                   .groupby(events_df['event_timestamp'].dt.date)
                   .size()
                   .reset_index(name='organic_android'))
organic_android['date'] = pd.to_datetime(organic_android['event_timestamp'])

# Merge into daily dataframe
daily = daily.merge(organic_ios[['date', 'organic_ios']], on='date', how='left')
daily = daily.merge(organic_android[['date', 'organic_android']], on='date', how='left')
daily.fillna(0, inplace=True)

# Cross-correlation for each platform
from statsmodels.tsa.stattools import ccf
import matplotlib.pyplot as plt
import numpy as np

for platform in ['ios', 'android']:
    ccf_vals = ccf(daily[f'organic_{platform}'], daily['total_paid_spend'], adjusted=False)
    plt.figure(figsize=(10,4))
    plt.stem(range(21), ccf_vals[:21])
    plt.xlabel('Lag (days)')
    plt.ylabel('Cross-correlation')
    plt.title(f'Organic {platform.capitalize()} Installs vs Total Paid Spend')
    plt.show()
...
# Assume `metrics` contains all channels with their metrics
# For clarity, filter only paid channels (excluding Organic, and maybe Referral if it's a different type)
paid_channels = metrics[metrics['channel'].isin(['Google App Campaigns', 'Google Brand Search', 
                                                 'Google Non-Brand Search', 'Meta', 'TikTok', 'Referral'])]

plt.figure(figsize=(10,6))
sns.scatterplot(data=paid_channels, x='CTR', y='conversion_rate', size='total_spend', 
                sizes=(50,500), hue='channel', legend=False, alpha=0.7)

# Add labels for each point
for _, row in paid_channels.iterrows():
    plt.text(row['CTR'], row['conversion_rate'], row['channel'], fontsize=9, ha='center')

plt.xlabel('Click-Through Rate (CTR)')
plt.ylabel('Install → Registration Conversion Rate (%)')
plt.title('Channel Positioning in the Acquisition Funnel')
plt.grid(True)
plt.show()
...
def label_campaign_type(name):
    name_lower = str(name).lower()
    if 'brand' in name_lower:
        return 'Brand'
    elif 'awareness' in name_lower or 'prospecting' in name_lower:
        return 'Awareness'
    elif 'retarget' in name_lower:
        return 'Retargeting'
    elif 'non-brand' in name_lower or 'search' in name_lower:
        return 'Performance'
    else:
        return 'Other'

spend_df['campaign_type'] = spend_df['campaign_name'].apply(label_campaign_type)

# Aggregate by campaign_type
type_metrics = spend_df.groupby('campaign_type').agg({
    'spend': 'sum',
    'impressions': 'sum',
    'clicks': 'sum'
}).reset_index()
type_metrics['CTR'] = type_metrics['clicks'] / type_metrics['impressions'] * 100
type_metrics['CPM'] = (type_metrics['spend'] / type_metrics['impressions']) * 1000
print(type_metrics)
...
# Assume I have:
# users_df (with registration_date and user_id)
# transactions_df (with user_id, transaction_timestamp, running_balance_usd)

# Merge registration date with users
reg_users = users_df[['user_id', 'registration_date']].copy()
reg_users['registration_date'] = pd.to_datetime(reg_users['registration_date'])

# Merge with transactions
user_trans = pd.merge(transactions_df, reg_users, on='user_id', how='inner')
user_trans['days_since_reg'] = (user_trans['transaction_timestamp'] - user_trans['registration_date']).dt.days

# I'll compute for each user the daily balance for the first 180 days
# Create a date range for each user? That's heavy. Instead, I can use the running_balance after each transaction and approximate.
# A simpler method: use the average of running_balance at the end of each day.
# I'll resample to daily for each user.

# For performance, I can process one user at a time, but I'll show a vectorized approach.

# First, ensure transactions are sorted by user and timestamp
user_trans = user_trans.sort_values(['user_id', 'transaction_timestamp'])

# Function to compute average daily balance over first N days
def compute_avg_daily_balance(group, max_days=180):
    # group has transactions for one user, sorted
    if len(group) == 0:
        return 0
    # registration date
    reg_date = group['registration_date'].iloc[0]
    # end date = reg_date + max_days
    end_date = reg_date + pd.Timedelta(days=max_days)
    # Create a daily date range from reg_date to end_date
    dates = pd.date_range(reg_date, end_date, freq='D')
    # Get the running balance at each date (forward fill from last transaction before that date)
    balances = []
    for d in dates:
        # transactions up to this date
        prev_trans = group[group['transaction_timestamp'] <= d]
        if len(prev_trans) > 0:
            bal = prev_trans.iloc[-1]['running_balance_usd']
        else:
            bal = 0  # no transactions yet
        balances.append(bal)
    return np.mean(balances)

# Apply to each user (slow, but okay for moderate number of users)
# I'll loop over users
user_ids = reg_users['user_id'].unique()
ltv_records = []
for uid in user_ids:
    user_data = user_trans[user_trans['user_id'] == uid]
    if len(user_data) == 0:
        avg_bal = 0
    else:
        avg_bal = compute_avg_daily_balance(user_data, max_days=180)
    # fee revenue over 180 days
    daily_fee_rate = 0.005 / 365  # 0.5% annual
    fee_revenue_180 = avg_bal * daily_fee_rate * 180
    ltv_records.append({'user_id': uid, 'avg_daily_balance_180': avg_bal, 'ltv_180': fee_revenue_180})
...
# MAGIC %md
# MAGIC ### Early LTV Prediction (First 14 Days

# COMMAND ----------

# Get install events for all users (including those who may not have registered)
installs = events_df[events_df['event_name'] == 'install'].copy()
installs['install_date'] = installs['event_timestamp'].dt.date  # or keep as datetime

# For each user, get their install info (first install? assume one per user)
# We'll take the earliest install per user
user_install = installs.sort_values('event_timestamp').groupby('user_id').first().reset_index()

# Merge with LTV (only registered users)
model_df = pd.merge(user_install, ltv_df, on='user_id', how='inner')  # inner because LTV only for registered

# Now add first-14-day features from transactions
# First, get all transactions in first 14 days after install
model_df['install_timestamp'] = pd.to_datetime(model_df['event_timestamp'])
transactions_df['transaction_timestamp'] = pd.to_datetime(transactions_df['transaction_timestamp'])

# Merge transactions with model_df to get per-user transactions
user_trans_14 = pd.merge(transactions_df, model_df[['user_id', 'install_timestamp']], on='user_id', how='inner')
user_trans_14['days_since_install'] = (user_trans_14['transaction_timestamp'] - user_trans_14['install_timestamp']).dt.days
early_trans = user_trans_14[user_trans_14['days_since_install'] <= 14]

# Aggregate per user
early_features = early_trans.groupby('user_id').agg(
    deposit_count_14=('transaction_type', lambda x: (x == 'deposit').sum()),
    withdrawal_count_14=('transaction_type', lambda x: (x == 'withdrawal').sum()),
    total_deposit_14=('amount_usd', lambda x: x[early_trans['transaction_type'] == 'deposit'].sum()),
    total_withdrawal_14=('amount_usd', lambda x: x[early_trans['transaction_type'] == 'withdrawal'].sum()),
    avg_balance_14=('running_balance_usd', 'mean'),  # average of running balance over those transactions (approx)
).reset_index()
early_features['net_deposit_14'] = early_features['total_deposit_14'] - early_features['total_withdrawal_14']

# Merge back
model_df = pd.merge(model_df, early_features, on='user_id', how='left').fillna(0)

# Add registration within 14 days flag
model_df['registered_within_14'] = (model_df['registration_date'] <= model_df['install_timestamp'] + pd.Timedelta(days=14)).astype(int)

# Add demographics (already have from user_profiles, but we need to merge)
model_df = pd.merge(model_df, users_df[['user_id', 'age', 'gender', 'tracking_enabled']], on='user_id', how='left')

# Add channel, platform, country from install event
model_df = pd.merge(model_df, installs[['user_id', 'source', 'platform', 'country']], on='user_id', how='left')

# One-hot encode categoricals
model_df = pd.get_dummies(model_df, columns=['source', 'platform', 'country', 'gender'], dummy_na=False)
...
features = [col for col in model_df.columns if col not in ['user_id', 'ltv_180', 'install_timestamp', 'registration_date', 'event_timestamp']]
X = model_df[features]
y = model_df['ltv_180']
...
# Get predicted LTV for all users in model_df (or use a separate test set)
model_df['predicted_ltv'] = np.expm1(rf.predict(X))

# Group by source
channel_quality = model_df.groupby('source').agg(
    avg_pred_ltv=('predicted_ltv', 'mean'),
    count=('user_id', 'count')
).reset_index()

# Merge with CAC from Part 1 (assuming we have a metrics DataFrame)
# metrics has columns: source, CAC, CPI, etc.
channel_quality = pd.merge(channel_quality, metrics[['source', 'CAC', 'CPI']], on='source', how='left')

# Compute LTV/CAC ratio
channel_quality['LTV_CAC_ratio'] = channel_quality['avg_pred_ltv'] / channel_quality['CAC']

# Payback period (months) using 180-day LTV as 6-month value
channel_quality['payback_months'] = (channel_quality['CAC'] * 6) / channel_quality['avg_pred_ltv']

print(channel_quality.sort_values('LTV_CAC_ratio', ascending=False))

# COMMAND ----------

# MAGIC %md
# MAGIC ## iOS Estimation Using SKAN Data

# COMMAND ----------

# On Android users (from model_df filtered by platform_android)
android_users = model_df[model_df['platform_android'] == 1]  # assuming one-hot

# Define conversion value based on total_deposit_14
# Use quantiles to create 64 bins (0-63)
android_users['conv_value'] = pd.qcut(android_users['total_deposit_14'], q=64, labels=False, duplicates='drop')
# If many zeros, the first bin will be zero, etc.

# Average LTV per conversion value
ltv_per_conv = android_users.groupby('conv_value')['ltv_180'].mean().reset_index(name='avg_ltv')

# Now for iOS, we have skan_df
# skan_df has: postback_date, network, campaign_id, skan_conversion_value, install_count
# Merge with spend_df to get spend per campaign (maybe need campaign_id mapping)
skan_with_ltv = pd.merge(skan_df, ltv_per_conv, left_on='skan_conversion_value', right_on='conv_value', how='left')
skan_with_ltv['estimated_ltv'] = skan_with_ltv['install_count'] * skan_with_ltv['avg_ltv']

# Aggregate by network (channel)
ios_channel_value = skan_with_ltv.groupby('network').agg(
    total_estimated_ltv=('estimated_ltv', 'sum'),
    total_installs=('install_count', 'sum')
).reset_index()

# Merge with spend on iOS campaigns? We need iOS spend per channel.
# spend_df has channel, but we need to know which campaigns are iOS. SKAN only applies to iOS, so spend on those campaigns is iOS spend.
# We can aggregate spend by channel from spend_df, but careful: some campaigns may have both iOS and Android spend. However, SKAN only covers iOS installs. We can assume spend is allocated proportionally or just use total spend as a proxy.
# Better: get iOS-specific spend if available (not in data). So we'll use total spend per channel for comparison.

# Merge with spend
channel_spend = spend_df.groupby('channel')['spend'].sum().reset_index()
ios_channel_value = pd.merge(ios_channel_value, channel_spend, left_on='network', right_on='channel', how='left')

# Compute estimated LTV/CAC
ios_channel_value['est_LTV_CAC'] = ios_channel_value['total_estimated_ltv'] / ios_channel_value['spend']
